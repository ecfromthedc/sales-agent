...
// imported cross-Worker without dragging in the concrete Env (and, via Env,
// @cloudflare/puppeteer). The DB-specific CRUD helpers below stay here — they
// read the NOTION_*_DB_ID vars off the concrete Env and are sales-only.
//
// Re-export the transport so existing callers importing these names from this
// module (e.g. the src/shared barrel, prior to SALE-133) keep working unchanged.
export { notionFetch } from "./notion-transport";
export type { NotionEnv, NotionFetchOptions } from "./notion-transport";
